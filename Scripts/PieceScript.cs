    ...
    void OnMouseUp()
{
    ...
    if (movePlate != null)
    {
      ...
      if (!game.promotionInProgress && game.gamePosition.GetPlayer() == game.aiPlayer)
    {
        game.DelayedAIMove();
    }
      //game.gamePosition.ConsoleLogPosition();
      //game.ConsoleLogGame();
}
...
public void HandleMoveOnBoardAndLogic(Move moveMade){
   if (!moveMade.isAttack && moveMade.myPiece.GetPlayerColor() == "white"){
      game.audioSource.PlayOneShot(game.whiteMoveSound);
    }
    if (!moveMade.isAttack && moveMade.myPiece.GetPlayerColor() == "black"){
      game.audioSource.PlayOneShot(game.blackMoveSound);
    }
    if (moveMade.isAttack){
      game.audioSource.PlayOneShot(game.takeSound);
    }
    game.gamePosition.HandleMove(moveMade);
    HandleMove(moveMade);
    game.SetPositionEmpty(moveMade.startingX, moveMade.startingY); // Clear old position
    string fen = game.gamePosition.GenerateFENForRepetitionTracking();
    game.gamePosition.AddFENToHistory(fen);
    game.CheckDrawOrCheckMate();
    game.PrintEvaluation();
}
...
{
    game.SetPositionEmpty(myPiece.GetBoardX(), myPiece.GetBoardY()); // Clear old position
...
private void HandleMove(Move moveMade)
{
    ...
    if ((myPiece.name == "white_pawn" && moveMade.GetBoardY() == 7) || (myPiece.name == "black_pawn" && moveMade.GetBoardY() == 0)) {
      ...
      if (myPiece.GetPlayerColor() == game.aiPlayer){
        HandleAIPromotion(moveMade);
      }
      else{
...
private void HandleAttack(Move moveMade)
{
    ...
    if (attackedPieceScript != null)
    {
        ...
        game.SetPositionEmpty(targetX, targetY);
...
public void HandleAIPromotion(Move moveMade)
{
    ...
    {
      UnityEngine.Debug.Log("Handle AI Promotion ran");   // AI promotion
      game.CompleteAIPromotion(moveMade.myPiece, "black_queen");
    }
}
